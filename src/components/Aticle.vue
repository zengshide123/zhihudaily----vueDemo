        ...
        <div class="daily-comments" v-show="comments.length">
             ...
             <div class="daily-comment" v-for="(comment,index) in comments" :key="index">
                <div class="daily-comment-avatar">
                    <img :src="comment.avatar" alt="per-icon">    
                </div>  
...
export default {
    ...
    methods:{
        getArticle(){
            Utils.ajax.get('news/'+this.id).then(res=>{
                res.body = res.body.replace(/src="http/g,'src="'+Utils.imgPath+'http');
                res.body = res.body.replace(/src="https/g,'src="'+Utils.imgPath+'https');
                this.data = res;
                window.scrollTo(0,0);    
            })
        },
        getComments(){
            this.comments = [];
            Utils.ajax.get('story/'+this.id+'/short-comments').then(res=>{
                this.comments = res.comments.map(comment=>{
                    comment.avatar = Utils.imgPath + comment.avatar;
                    return comment
                })
            })
        }
